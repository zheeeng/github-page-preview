...
const getRandomEmoji = (x, y, w, h) => ({
    code: getRandomEmojiCode(),
    opacityFadeSpeed: Math.random() * 0.05 + 0.9,
    xMoveSpeed: (Math.random() * 0.01 - 0.005) * w,
    yMoveSpeed: (Math.random() * 0.01 - 0.005) * h,
    opacity: 1,
    x,
    y,
    end: false,
...
const getNextEmoji = emoji => Object.assign(
    emoji,
    {
        opacity: emoji.opacity * emoji.opacityFadeSpeed,
        x: emoji.x + emoji.xMoveSpeed,
        y: emoji.y + emoji.yMoveSpeed,
        end: emoji.opacity < 0.1,
    },
)
...
const body = dom`
    <slot></slot>
    <canvas id="canvas" on-click="cheer"></canvas>
`
...
* @extends HTMLElement
*/
class CheerBG extends PolymerElement {
    ...
    cheer(event) {
        ...
        const emojis = Array.from(
            { length: this.maximum },
            () => getRandomEmoji(offsetX, offsetY, ...this.canvasSize),
        )

        const drawIter = this.emojisDrawer(emojis)
        const draw = () => {
            window.requestAnimationFrame(() => {
                const { done } = drawIter.next()
                !done && draw()
            })
        }
    ...
    clear() {
        this.canvasContext.clearRect(0, 0, ...this.canvasSize)
    }

    * emojisDrawer(emojis) {
        while (!emojis.every(emoji => emoji.end)) {
            this.clear()
            ...
            emojis.forEach(emoji => {
                ...
                this.canvasContext.globalAlpha = nextEmoji.opacity
                this.canvasContext.font = '48px serif'
                this.canvasContext.fillText(nextEmoji.code, nextEmoji.x, nextEmoji.y)
            })
...
customElements.define('i-cheer-bg', CheerBG)
